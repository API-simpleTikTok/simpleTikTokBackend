...
public class SecurityConstant
{
    ...
    public void init()
    {
        ...
        for (User user : users)
        {
            USER_MAP.put(user.getAuthor(), user.getPassword());

            if(user.getAuthor().equals("admin"))
            {
                //赋予当前用户管理员权限
                USER_PERMISSION_MAP.put(user.getAuthor(), List.of(PERMISSION.ADMIN, PERMISSION.USER));
            }
            else
            {
                //普通用户
                USER_PERMISSION_MAP.put(user.getAuthor(), List.of(PERMISSION.USER));
            }
        }
    }
